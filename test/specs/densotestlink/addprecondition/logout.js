describe("adding test cases on test link", async () => {
    ...
    const preCondition = 'User must be on logged in'
    ...
    it("expand testsuite", async () => {
      ...
      const testSuite = await $(
        '//span[text()="Logout (10)"]/parent::span/parent::a/parent::div/img[1]'
      );
    ...
    it("click on test case and start writing basic info", async () => {
      ...
      const testCases = await $$(
        '//span[text()="Logout (10)"]/parent::span/parent::a/parent::div/parent::li/ul/li'
      );
      ...
      for (let ele of testCases) {
        ...
        await preConditionInput.scrollIntoView({inline: 'center'})
        ...
        // click on Resequence steps
        const resequenceSteps = await $('input[name="resequence_steps"]')
        await resequenceSteps.waitForDisplayed()
        await resequenceSteps.click()
